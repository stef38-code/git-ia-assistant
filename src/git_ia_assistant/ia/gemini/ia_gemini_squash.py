...
"""
NAME
    ia_gemini_squash - Suggestion de squash IA avec Gemini

DESCRIPTION
    Classe pour générer une suggestion de squash de commits via Gemini.

FUNCTIONS
    recuperer_commits() -> list
        Récupère les derniers commits à traiter.
    generer_prompt(commits: list) -> str
        Génère le prompt détaillé pour Gemini avec la liste des commits.
    generer_squash(commits: list) -> str
        Envoie le prompt à Gemini et retourne la suggestion de squash.
...
class IaGeminiSquash(IaAssistantSquash):
    """
    Suggestion de squash IA via Gemini.
    """
    ...
    def generer_squash(self, liste_commits: list) -> str:
        ...
        """
        modele_prompt = charger_prompt("commits/squash_prompt.md", self.dossier_prompts)
