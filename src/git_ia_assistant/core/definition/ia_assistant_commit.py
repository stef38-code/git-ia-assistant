    ...
    def detecter_fichiers(self):
        if not self.fichiers:
            ...
            for f in self.fichiers:
                # Créer un chemin absolu pour une vérification fiable
                abs_path = os.path.join(repo_root, f)
                
                # Vérifier si le fichier existe OU s'il est connu de Git (cas d'une suppression)
                existe_sur_disque = os.path.exists(abs_path)
                est_suivi_par_git = False
                try:
                    # On vérifie si le fichier est dans l'index ou le dernier commit
                    self.repo.git.ls_files('--error-unmatch', f)
                    est_suivi_par_git = True
                except git.exc.GitCommandError:
                    est_suivi_par_git = False

                if existe_sur_disque or est_suivi_par_git:
                    # Vérifier si le fichier ou son répertoire parent est ignoré par .gitignore
                    est_ignore = False
                    try:
                        # Vérifier le fichier lui-même
                        self.repo.git.check_ignore(f)
                        est_ignore = True
                    except git.exc.GitCommandError:
                        # Vérifier aussi le répertoire parent (cas .idea/, .venv/, etc.)
                        if '/' in f:
                            premier_dir = f.split('/')[0] + '/'
                            try:
                                self.repo.git.check_ignore(premier_dir)
                                est_ignore = True
                            except git.exc.GitCommandError:
                                pass
                    
                    # Si le fichier ou son répertoire est ignoré, le sauter
                    if est_ignore:
                        logger.log_debug(True, f"Le fichier '{f}' est ignoré par .gitignore et sera sauté.")
                        continue
                    
                    fichiers_a_ajouter.append(f)
                else:
                    # Avertir si un fichier n'est ni sur disque ni connu de Git, puis l'ignorer
                    logger.log_warn(f"Le fichier '{f}' n'a pas été trouvé sur le disque et n'est pas suivi par Git. Ignoré.")

            if fichiers_a_ajouter:
                try:
                    # Tester d'abord avec --dry-run pour détecter les fichiers ignorés
                    # que check-ignore ne détecte pas toujours
                    try:
                        self.repo.git.add('--dry-run', '--', *fichiers_a_ajouter)
                    except git.exc.GitCommandError as e:
                        # Si l'erreur mentionne des fichiers ignorés, les filtrer
                        if "ignored by one of your .gitignore" in str(e) or "are ignored" in str(e):
                            logger.log_warn("Certains fichiers sont ignorés par .gitignore et seront automatiquement filtrés.")
                            # Filtrer en testant chaque fichier individuellement
                            fichiers_valides = []
                            for f in fichiers_a_ajouter:
                                try:
                                    self.repo.git.add('--dry-run', '--', f)
                                    fichiers_valides.append(f)
                                except git.exc.GitCommandError:
                                    logger.log_debug(True, f"Le fichier '{f}' a été ignoré par Git.")
                            fichiers_a_ajouter = fichiers_valides
                        else:
                            raise
                    
                    # Ajouter réellement les fichiers valides
                    if fichiers_a_ajouter:
                        self.repo.git.add(fichiers_a_ajouter)
                except git.exc.GitCommandError as e:
                    logger.log_error(f"Erreur Git lors de l'ajout/suppression des fichiers : {e}")
            else:
                logger.log_info("Aucun changement valide à ajouter.")
